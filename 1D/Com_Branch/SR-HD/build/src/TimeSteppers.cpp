...
void HybridDetection(double *Prims, bool *Det, int Start, int Stop,
                     const int rcm_Counter) {
  double DivP, DivV, DivR;

  const int Radius = 2;
  for (int i = Start; i < Stop; ++i) {
    // DivP = std::fabs((1./12.)*Prims[Tidx(PRES,i-2)] -
    // (2./3.)*Prims[Tidx(PRES,i-1)]
    //  + (2./3.)*Prims[Tidx(PRES,i+1)] - (1./12.)*Prims[Tidx(PRES,i+2)]);

    DivP = std::fabs(Prims[Tidx(PRES, i + 1)] - Prims[Tidx(PRES, i - 1)]) /
           std::fmin(Prims[Tidx(PRES, i + 1)], Prims[Tidx(PRES, i - 1)]);

    //  DivR = std::fabs((1./12.)*Prims[Tidx(DENS,i-2)] -
    //  (2./3.)*Prims[Tidx(DENS,i-1)]
    //  + (2./3.)*Prims[Tidx(DENS,i+1)] - (1./12.)*Prims[Tidx(DENS,i+2)]);

    DivR = std::fabs(Prims[Tidx(DENS, i + 1)] - Prims[Tidx(DENS, i - 1)]) /
           std::fmin(Prims[Tidx(DENS, i + 1)], Prims[Tidx(DENS, i - 1)]);
...
void Domain::RK2() {
  ...
  for (int i = 3; i < REdgeX - 3; ++i) {
    // Pressure
    // DivP[i] = (std::fabs((1./12.)*Prims[Tidx(PRES,i-2)] -
    // (2./3.)*Prims[Tidx(PRES,i-1)]
    //  + (2./3.)*Prims[Tidx(PRES,i+1)] - (1./12.)*Prims[Tidx(PRES,i+2)]));

    // DivP[i] = std::fabs(Prims[Tidx(PRES,i+1)] -
    // Prims[Tidx(PRES,i-1)])/(std::fmin(Prims[Tidx(PRES,i+1)],
    // Prims[Tidx(PRES,i-1)]));

    //  Dens
    //  DivP[i] = (std::fabs((1./12.)*Prims[Tidx(DENS,i-2)] -
    //  (2./3.)*Prims[Tidx(DENS,i-1)]
    //  + (2./3.)*Prims[Tidx(DENS,i+1)] - (1./12.)*Prims[Tidx(DENS,i+2)]))/dx;

    DivP[i] = std::fabs(Prims[Tidx(DENS, i + 1)] - Prims[Tidx(DENS, i - 1)]) /
              (std::fmin(Prims[Tidx(DENS, i + 1)], Prims[Tidx(DENS, i - 1)]));
...
void Domain::RK4() {
  ...
  double a20 = 0.444370493651235, a21 = 0.555629506348765,
         c2 = 0.368410593050371;
  double a30 = 0.620101851488403, a32 = 0.379898148511597,
         c3 = 0.251891774271694;
  double a40 = 0.178079954393132, a43 = 0.821920045606868,
         c4 = 0.544974750228521;
  ...
  // DomainCopy(Cons, U1); // U1 and CopyBuffer, and Cons Fine ForwardEuler();
  // // Cons = Fl, CopyBuffer = U0, U1 = U1 DomainAdd(a21, c2, U1, Cons);
  // DomainAdd(a20, 1.0, CopyBuffer, Cons); // Cons = U2, CopyBuffer = U0, U1 =
  // U1 DomainCopy(Cons, U1);  // Cons = U2, CopyBuffer = U0, U1 = U2
  // ForwardEuler();  // Cons = Fl, CopyBuffer = U0, U1 = U2
  // DomainCopy(U1, UNew); // Cons = Fl, CopyBuffer = U0, U1 = U2, UNew = U2 //
  // U1, CopyBuffer, Cons, UNew DomainAdd(c3, a32, Cons, U1);
  // DomainAdd(a30, 1.0, CopyBuffer, U1); // Cons = Fl, CopyBuffer = U0, U1 =
  // U3, UNew = U2 DomainCopy(U1, Cons); // Cons = U3, CopyBuffer = U0, U1 = U3,
  // UNew = U2 DomainAdd(f3, f2, U1 , UNew); // Cons = U3, CopyBuffer = U0, U1 =
  // U3, UNew = f2U2 + f3U3 ForwardEuler();  // Cons = FU3, CopyBuffer = U0, U1
  // = U3, UNew = f2U2 + f3U3 DomainAdd(ff3, 1.0, Cons, UNew); // Cons = FU3,
  // CopyBuffer = U0, U1 = U3, UNew = f2*U2 + f3*U3 + ff3*FU3 DomainAdd(a43, c4,
  // U1, Cons); DomainAdd(a40, 1.0, CopyBuffer, Cons); // Cons = U4, CopyBuffer
  // = U0, U1 = U3, UNew = f2*U2 + f3*U3 + ff3*FU3 DomainCopy(Cons, U1); // Cons
  // = U4, CopyBuffer = U0, U1 = U4, UNew = f2*U2 + f3*U3 + ff3*FU3
  // DomainAdd(f4, 1.0, Cons, UNew); // Cons = U4, CopyBuffer = U0, U1 = U4,
  // UNew = f2*U2 + f3*U3 + ff3*FU3 + f4*U4 ForwardEuler(); // Cons = FU4,
  // CopyBuffer = U0, U1 = U4, UNew = f2*U2 + f3*U3 + ff3*FU3 + f4*U4
  ...
  for (int i = 0; i < iter; ++i) {
    Cons[i] =
        f2 * U2[i] + f3 * U3[i] + ff3 * FU3[i] + f4 * U4[i] + ff4 * FU4[i];
  }
}
