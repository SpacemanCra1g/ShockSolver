...
#define GP_METHOD
    ...
    BC = &Domain::NeumannBC;
#elif TestProblem == SHOCKONLY
    IC = &Domain::ShockOnly;
    BC = &Domain::NeumannBC;
#elif TestProblem == CONTACTONLY
    IC = &Domain::ContactOnly;
    BC = &Domain::NeumannBC;
#elif TestProblem == WENONE
    IC = &Domain::WenProblem1;
    BC = &Domain::NeumannBC;
  ...
  // Defined in the Physics Dependent VarConvert.cpp file
  void Prims2Cons(double *, double *, int, int);
  int Cons2Prim(double *, double *, int, int);

  // I don't actually think these functions exist anymore
  void SolvePressure();
  void Press(int x);
  ...
  // Defined in the Physics dependent Find_dt.cpp flie
  void Find_dt();
  ...
  void HLL_Flux(double *Dest, double *PrL, double *PrR, double SL, double SR,
                int i);
