...
for t in range(2,-1,-1):
  for i in range(3):
    for j in range(3,):
      ...
      ax[i][j].plot(x,Pres/1000,Lines[t],linewidth=3)
      line[t], = ax[i][j].plot(x,Dens/25,Lines[t],linewidth=3,label=Labels[t])
      ax[i][j].plot(x,Xvel,Lines[t],linewidth=3)
...
# print(line)
plt.suptitle("Comparison of the exact solution v.s. the Hybrid and HLLC+WENO solutions",fontsize=20,y=.95)
plt.legend(handles=[t for t in line],fontsize=18)
