  ...
  void SampleRarefaction(double S, double Out[3]){
    ...
    switch (dir){
      case -1:
        ...
        Out[0] = Left->rho*pow(2.0/(GAMMA+1.0) + ((GAMMA-1.0)/((GAMMA+1.0)*a))*(Left->v - S) ,2.0/(GAMMA-1.0));
        
        Out[1] = (2.0/(GAMMA+1.0))*(a + .5*(GAMMA-1.0)*Left->v + S);
        
        Out[2] = Left->p*pow(2.0/(GAMMA+1.0) + ((GAMMA-1.0)/((GAMMA+1.0)*a))*(Left->v - S) ,2.0/(GAMMA-1.0));
        break;
      case 1:
        a = sqrt(GAMMA*Right->p/Right->rho);
        Out[0] = Right->rho*pow(2.0/(GAMMA+1.0) - ((GAMMA-1.0)/((GAMMA+1.0)*a))*(Right->v - S) ,2.0/(GAMMA-1.0));
        
        Out[1] = (2.0/(GAMMA+1.0))*(-a + .5*(GAMMA-1.0)*Right->v + S);
        
        Out[2] = Right->p*pow(2.0/(GAMMA+1.0) - ((GAMMA-1.0)/((GAMMA+1.0)*a))*(Right->v - S) ,2.0/(GAMMA-1.0));
        break;
    };
  };
  ...
  void init(State *LeftSide, State *RightSide, int Dir){
    ...
    case -1:
      if (WaveType=="Shock"){
        ...
        RL = Left->v - a;
        RR = Right->v - a*pow(Right->p/Left->p,(GAMMA-1.0)/(2.*GAMMA) );
      } else {
    ...
    case 1:
      if (WaveType=="Shock"){
        ...
        RR = Right->v + a;
        RL = Left->v + a*pow(Left->p/Right->p,(GAMMA-1.0)/(2.*GAMMA) );
      } else {
...
double FindP_Star(const State &Left, const State &Right) {
  ...
  double p_star_new = p_star - (f(Left,p_star) + f(Right,p_star) + (Right.v - Left.v) )/(fprime(Left,p_star) + fprime(Right,p_star));
  ...
  while (Difference > 1.e-12 && index < 50){
    p_star = p_star_new;
    p_star_new = p_star - (f(Left,p_star) + f(Right,p_star) + (Right.v - Left.v) )/(fprime(Left,p_star) + fprime(Right,p_star));
...
void FindRho_Star(State Left, State Right, double p_star, double &Rho_L, double &Rho_R, Wave &LeftWave, Wave &RightWave){
  double pval, gam;
  if (p_star > Left.p){
    pval = p_star/Left.p;
...
class RiemannFan{
  ...
  RiemannFan(double L[3],double R[3]){
    ...
    FindRho_Star( Left,  Right,  Wave3.p, Wave3.rho,Wave4.rho, LeftWave,RightWave );
...
void ExactSample(double Left[3],double Right[3], double Out[3], double S){
    RiemannFan Fan(Left,Right);
    Fan.SampleSolution(S, Out);
    return;
}

// int main(){
//   double T = .01;
//   double L[3] = {1.0,0.0,1000.};
//   double  R[3] = {1.0,0.0,0.01};
